...
| Endpoint | Method | Purpose | Needs `stock_codes`? |
|---|---|---|---|
| `/analyze/query` | POST | Natural-language query (auto-resolves stock) | No -- stock extracted from `query` |
| `/analyze` | POST | Smart analysis (NL / multi-turn / direct) | Yes |
| `/analyze/full` | POST | Full quick analysis (all feature groups) | Yes |
| `/analyze/manifest` | GET | List all feature groups | No |
| `/analyze/sessions/{id}` | GET | Query session status | No |
| `/health` | GET | Health check + cache stats | No |

### Key Request Fields

| Field | Type | Required | Description |
|---|---|---|---|
| `query` | string | On `/analyze/query` | Natural-language question, e.g. "帮我看看建科智能怎么样" |
| `stock_codes` | string[] | On `/analyze`, `/analyze/full` | Stock codes, e.g. `["600519"]` |
| `feature_groups` | string[] | No | Directly specify groups; skips LLM reasoning |
| `session_id` | string | No | Multi-turn session ID |
| `selected_option` | int | No | Index of user-chosen option (for clarification flow) |
| `skip_clarification` | bool | No | Default `true` -- auto-selects full analysis when intent is ambiguous |
| `ohlcv_count` | int | No | Daily candle count (100-5000, default 300) |
| `technical_count` | int | No | 60-min candle count (400-5000, default 800) |

### Available Feature Groups

| Group | Features | Description |
|---|---|---|
| `technical` | 212 | RSI / MACD / KDJ / Bollinger / momentum ... |
| `flow_daily` | 64 | MFS / MFI / capital concentration (daily) |
| `flow_minute` | 16 | Early-session / late-session / washout (intraday) |
| `flow_combined` | 80 | Minute + daily combined |
| `ohlcv_combined` | 42 | K-patterns + lifeline + experience |
| `k_patterns` | 20+ | Engulfing / doji / hammer ... |
| `lifeline` | -- | Red-blue bands / buy-sell signals |
| `experience` | -- | Price change / volume trend |

### Response Status Codes

| `status` value | Meaning |
|---|---|
| `success` | Analysis complete; report in `results.{code}.report` |
| `needs_clarification` | Intent ambiguous; see `clarification_question` + `suggested_options` |
| HTTP 400 | Could not resolve stock from query |
...
#### Step 1 -- Save the OpenAPI Schema

Save the following as `astockd_openapi.yaml`:

```yaml
openapi: 3.0.0
info:
  title: AStockD Analysis Engine
  description: >
    A-share technical / capital-flow / K-line pattern analysis with
    natural-language stock resolution and multi-turn clarification.
  version: "0.2.0"
...
paths:
  /analyze/query:
    post:
      operationId: stockDoctor
      summary: Natural-language query (auto-resolves stock)
      description: |
        Users speak naturally, e.g. "帮我看看建科智能怎么样".
        Resolution: dictionary match -> vector similarity -> regex extraction.
        skip_clarification defaults to true; ambiguous intent auto-selects full analysis.
      requestBody:
        required: true
        content:
          application/json:
            schema:
              ...
              properties:
                ...
                skip_clarification:
                  type: boolean
                  default: true
                  description: Skip clarification and auto-select full analysis (recommended for agents)
                ohlcv_count:
                  type: integer
                  default: 300
                technical_count:
              ...
              schema:
                ...
                properties:
                  ...
                  results:
                    type: object
        "400":
          description: Could not resolve stock

  /analyze:
    post:
      operationId: stockDiagnosis
      summary: Smart stock analysis
      description: |
        Three modes:
        1. Natural language: pass query, engine reasons about feature groups
        2. Multi-turn: pass session_id + selected_option
        3. Direct: pass feature_groups to skip reasoning
      requestBody:
        required: true
        content:
          application/json:
                ...
                feature_groups:
                  ...
                  items:
                    ...
                    enum:
                      ...
                      - flow_daily
                      - flow_combined
                  description: Feature groups to compute
                session_id:
                  type: string
                selected_option:
                  type: integer
  ...
  /analyze/full:
    post:
      operationId: fullCheckup
      summary: Full quick analysis
      description: One-shot technical + capital-flow + pattern analysis
      requestBody:
        required: true
        content:
          application/json:
  ...
  /analyze/manifest:
    get:
      ...
      description: Returns metadata for all available feature groups
      responses:
        "200":
          description: Feature manifest
...
For any platform or framework that can make HTTP requests.
...
API_URL = "http://your-server:8888"

# Method 1: Natural-language query (auto-resolves stock) -- recommended
resp = httpx.post(f"{API_URL}/analyze/query", json={
    "query": "帮我看看建科智能怎么样"
})
result = resp.json()

if resp.status_code == 400:
    print("Could not resolve stock. Please provide a name or code.")
    exit()

# Method 2: Analysis with stock code
resp = httpx.post(f"{API_URL}/analyze", json={
    "stock_codes": ["600519"],
    "query": "主力资金动向"
})
result = resp.json()
...
if result["status"] == "needs_clarification":
    ...
    for opt in result["suggested_options"]:
        print(f"  [{opt['index']}] {opt['label']}: {opt['description']}")

    choice = int(input("Choose: "))
    resp2 = httpx.post(f"{API_URL}/analyze", json={
        "stock_codes": ["600519"],
...
```typescript
const API_URL = "http://your-server:8888";

async function analyzeByQuery(query: string) {
  const resp = await fetch(`${API_URL}/analyze/query`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
async function analyzeStock(stockCodes: string[], query?: string) {
  const resp = await fetch(`${API_URL}/analyze`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ stock_codes: stockCodes, query }),
  });
  return resp.json();
}

async function clarify(stockCodes: string[], sessionId: string, selectedOption: number) {
  const resp = await fetch(`${API_URL}/analyze`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
...
```bash
# Natural-language query (auto-resolves stock) -- recommended
curl -X POST http://localhost:8888/analyze/query \
  -H "Content-Type: application/json" \
  -d '{"query": "帮我看看建科智能怎么样"}'

# Analysis with stock code
curl -X POST http://localhost:8888/analyze \
  -H "Content-Type: application/json" \
  -d '{"stock_codes": ["600519"], "query": "主力资金动向"}'

# Direct feature-group specification
curl -X POST http://localhost:8888/analyze \
  -H "Content-Type: application/json" \
  -d '{"stock_codes": ["600519"], "feature_groups": ["technical", "flow_daily"]}'
