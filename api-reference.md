...
# Natural language query (easiest way)
def analyze_query(query):
    return requests.post(f"{BASE_URL}/analyze/query", json={
        "query": query,
    }).json()

# Smart analysis with stock codes
def analyze_stock(stock_codes, query=None):
    return requests.post(f"{BASE_URL}/analyze", json={
        "stock_codes": stock_codes,
        "query": query,
    }).json()

# Full analysis shortcut
def analyze_full(stock_codes):
    return requests.post(f"{BASE_URL}/analyze/full", json={
        "stock_codes": stock_codes,
    }).json()
...
# K-line patterns
def get_k_patterns(stock_codes):
...
# Combined money flow
def get_combined_flow(stock_codes):
    return requests.post(f"{BASE_URL}/flow/combined", json={
        "stock_codes": stock_codes,
    }).json()

# Combined money flow features (minute + daily + trends)
def get_combined_flow_features(stock_codes):
    return requests.post(f"{BASE_URL}/flow_features/combined", json={
        "stock_codes": stock_codes,
    }).json()

# All features (technical + money flow + OHLCV)
def get_all_features(stock_codes):
...
result = analyze_full(["600519"])
features = get_all_features(["000001"])
```
...
# Full analysis
curl -X POST http://your-server:8888/analyze/full \
  -H "Content-Type: application/json" \
  -d '{"stock_codes": ["600519"]}'

# K-line patterns
curl -X POST http://your-server:8888/features/k-patterns \
  -H "Content-Type: application/json" \
  -d '{"stock_codes": ["000001"], "frequency": "1d", "count": 300}'

# Combined money flow features
curl -X POST http://your-server:8888/flow_features/combined \
  -H "Content-Type: application/json" \
  -d '{"stock_codes": ["000001"]}'

# Batch stocks
curl -X POST http://your-server:8888/flow_features/daily \
  -H "Content-Type: application/json" \
  -d '{"stock_codes": ["000001", "600519", "SZ.300001"]}'

# All features
curl -X POST http://your-server:8888/features/all \
  -H "Content-Type: application/json" \
  -d '{"stock_codes": ["000001"]}'
```
